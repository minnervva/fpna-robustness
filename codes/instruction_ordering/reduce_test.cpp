...
__inline__ void dgemm(blas_handle_t handle__,
                      mdarray<double, 2, CblasRowMajor> &a__,
                      mdarray<double, 2, CblasRowMajor> &b__,
                      mdarray<double, 2, CblasRowMajor> &c__) {
  double alpha = 1.0;
  double beta = 1.0;
#ifdef REDUCE_USE_CUDA
  cublasDgemm(handle__,
              CUBLAS_OP_N, CUBLAS_OP_N,
              a__.size(0), b__.size(1), a__.size(1),
              &alpha,
              a__.at<device_t::GPU>(), a__.size(0),
              b__.at<device_t::GPU>(), b__.size(0),
              &beta,
              c__.at<device_t::GPU>(), c__.size(1));
#else
  hipblasDgemm(handle__,
...
template<class T> T variability(const T result__, const T reference_value__)
{
  return 1.0 - std::abs(result__ / reference_value__);
}
...
void performance_tests(cxxopts::ParseResult &result, rt_graph::Timer &timer__,
  ...
  if (!result["performance_tests"].as<bool>())
    return;
...
void allcoate_matrices(cxxopts::ParseResult &result,
                       mdarray<double, 2, CblasRowMajor> &a__,
                       mdarray<double, 2, CblasRowMajor> &b__,
                       mdarray<double, 2, CblasRowMajor> &c__) {
  const int sizem_ = result["matrix_size"].as<int>();
  a__ = mdarray<double, 2, CblasRowMajor>(sizem_, sizem_);
  b__ = mdarray<double, 2, CblasRowMajor>(sizem_, sizem_);
  c__ = mdarray<double, 2, CblasRowMajor>(sizem_, sizem_);
  a__.allocate(memory_t::device);
  b__.allocate(memory_t::device);
  c__.allocate(memory_t::device);
}

void measure_variability_under_load(cxxopts::ParseResult &result,
                                    rt_graph::Timer &timer__,
                                    std::vector<whip::stream_t> &vstream__)
{
  if (!result["variability_under_load"].as<bool>())
    return;

  fmt::print("┌{0:─^{2}}┐\n"
             "│{1: ^{2}}│\n"
             "└{0:─^{2}}┘\n\n",
             "", "Variablity under load", 70);

  fmt::print("Measure the variability when the GPU is under load with other tasks.\n"
             "Other load may impact the variability distribution as the scheduler and\n"
             "memory controller will run other instructions not linked to the reduction.\n"
             "Workload can use atomic operations or not. ideally we need to check both\n"
             "type of workload.");

  const int table_length__ = result["max_reduction_size"].as<int>();
  const int number_of_samples__ = 100;
  const int number_of_iterations__ = result["number_of_samples"].as<int>() / number_of_samples__;

  // matrices used for loading GPU with other tasks
  mdarray<double, 2, CblasRowMajor> a_, b_, c_;
  allcoate_matrices(result, a_, b_, c_);

  // rng used to generate random initial data for matrices and data for the reduction
  generator_t gen_;

  
  double *scratch_;
  whip::malloc(&scratch_, sizeof(double) * table_length__);
  mdarray<double, 2, CblasRowMajor> data_(number_of_samples__, table_length__);

  std::vector<double> det_sum_(number_of_samples__);
  std::vector<double> tmp_sum_(number_of_samples__);
  std::vector<double> res(result["number_of_samples"].as<int>());
  blas_handle_t bctx_;
  ...
  // the first stream is for the workload
  // the second stream for the reduction
  randSetStream(gen_, vstream__[0]);
  blasSetStream(bctx_, vstream__[0]);
  ...
  indicators::option::FontStyles{
    std::vector<indicators::FontStyle>{indicators::FontStyle::bold}}};
  bar.set_option(indicators::option::PostfixText{"Variability under load"});
  indicators::show_console_cursor(false);
  for (int sample = 0; sample < data_.size(0); sample++) {
    det_sum_[sample] = reduce(reduction_method::single_pass_gpu_det_shared,
                              vstream__[0],
                              64,
                              -1,
                              data_.size(1),
  ...
  whip::stream_synchronize(vstream__[0]);

  for (int sp = 0; sp < number_of_iterations__; sp++) {
    if ((sp % 100 == 0) && (sp != 0))
      bar.set_progress(100 * sp / number_of_iterations__);
    for (int sample = 0; sample < data_.size(0); sample++) {
      GenerateUniformDouble(gen_, a_.at<device_t::GPU>(), a_.size());
      GenerateUniformDouble(gen_, b_.at<device_t::GPU>(), b_.size());
      GenerateUniformDouble(gen_, c_.at<device_t::GPU>(), c_.size());
      dgemm(bctx_, a_, b_, c_);

      double tmp_res = reduce(reduction_method::single_pass_gpu_shared_atomic,
                              vstream__[1],
                              64,
                              -1,
                              data_.size(1),
                              data_.at<device_t::GPU>(sample, 0),
                              static_cast<double *>(scratch_));
      res[sp * 100 + sample] = variability(tmp_res, det_sum_[sample]);
    }
  }
  indicators::show_console_cursor(true);
  whip::stream_synchronize(vstream__[0]);
  whip::stream_synchronize(vstream__[1]);

  std::string filename_ = "variability_under_load_" + GPU_TYPE + ".csv";
  FILE *f = fopen(filename_.c_str(), "w+");
  for (int ms = 0; ms < res.size() - 1; ms++) {
    fprintf(f, "%.16e,", res[ms]);
  }
  fprintf(f, "%.16e\n", res.back());
  fclose(f);
...
void measure_atomic_ordering_under_load(cxxopts::ParseResult &result,
                                        const bool mapping_under_load_,
                                        rt_graph::Timer &timer__,
                                        std::vector<whip::stream_t> &vstream__)
{
d  if (mapping_under_load_) {
    fmt::print("┌{0:─^{2}}┐\n"
               "│{1: ^{2}}│\n"
               "└{0:─^{2}}┘\n\n",
               "", "Calculate the ordering of the atomicAdd under load", 70);

    fmt::print("Measure the order of the atomicAdd instruction when other loads are present.\n");

  } else {
    fmt::print("┌{0:─^{2}}┐\n"
               "│{1: ^{2}}│\n"
               "└{0:─^{2}}┘\n\n",
               "", "Calculate the ordering of the atomicAdd without load", 70);
    fmt::print("Measure the order of the atomicAdd instruction when the GPU only runs the reduction.\n");
  }
  

  const int number_of_elements_ = result["max_reduction_size"].as<int>();
  const int number_of_samples__ = 100;
  std::vector<double> res_(number_of_samples__);
  const int number_of_iterations__ = result["number_of_samples"].as<int>() / number_of_samples__;
  const int max_num_blocks_ = (number_of_elements_ + 128 - 1) / 128;
  generator_t gen_;
  blas_handle_t bctx_;
  ...
  mdarray<double, 2, CblasRowMajor> data_(number_of_samples__, number_of_elements_);
  mdarray<size_t, 2, CblasRowMajor> block_index_(number_of_samples__, max_num_blocks_);
  mdarray<double, 2, CblasRowMajor> scratch_(number_of_samples__, max_num_blocks_ + 1);

  data_.allocate(memory_t::device);
  scratch_.allocate(memory_t::device);

  CreateGenerator(&gen_, result["seed"].as<unsigned int>());
  randSetStream(gen_, vstream__[0]);

  timer__.start("generate_rng");
  // We choose the uniform distribution U(0, 1) as it simplify sorting afterwards
  ...
  for (int sp = 0; sp < number_of_iterations__; sp++) {
    ...
    for (int sample = 0; sample < data_.size(0); sample++) {
      ...
      res_[sample] = reduce_register_atomic(vstream__[1],
                                            number_of_elements_,
                                            data_.at<device_t::GPU>(sample, 0),
                                            scratch_.at<device_t::GPU>(sample, 0));
    }
    ...
    whip::stream_synchronize(vstream__[1]);
    scratch_.copy<memory_t::device, memory_t::host>();

    for (int sample = 0; sample < data_.size(0); sample++) {
      std::vector<double> to_be_sorted_(scratch_.at<device_t::CPU>(sample, 1), scratch_.at<device_t::CPU>(sample, 1) + max_num_blocks_);
      std::vector<size_t> block_index_sorted_(max_num_blocks_);
      sort_indexes<double>(to_be_sorted_, block_index_sorted_);
      memcpy(block_index_.at<device_t::CPU>(sample, 0), block_index_sorted_.data(), sizeof(size_t) * block_index_sorted_.size());
    }
    std::string filename_;
    if (mapping_under_load_) {
      filename_ = "atomic_ops_ordering_under_load_" + GPU_TYPE + "_" + std::to_string(a.size(0)) + ".csv";
    } else {
      if (result["mapping"].as<bool>())
        filename_ = "mapping_atomicInc_block_index_" + GPU_TYPE + "_" + std::to_string(a.size(0)) + ".csv";
    }
    FILE *f = nullptr;

    if ((sp == 0) && !result["append"].as<bool>()) {
       f = fopen(filename_.c_str(), "w+");
    } else {
      f = fopen(filename_.c_str(), "a+");
    }

    for (int sample = 0; sample < data_.size(0); sample++) {
      for (int bk = 0; bk < max_num_blocks_ - 1; bk++) {
        fprintf(f, "%d,", block_index_(sample, bk));
      }
      fprintf(f, "%d\n", block_index_(sample, max_num_blocks_ - 1));
    }
    fclose(f);

    if (mapping_under_load_) {
      filename_ = "sum_atomic_ops_ordering_under_load_" + GPU_TYPE + ".csv";
    } else {
      filename_ = "sum_mapping_atomicInc_block_index_" + GPU_TYPE + ".csv";
    }
    if ((sp == 0) && !result["append"].as<bool>()) {
      f = fopen(filename_.c_str(), "w+");
    } else {
    ...
    for (int sample = 0; sample < res_.size() - 1; sample++)
      fprintf(f, "%.15lf,", res_[sample]);
    fprintf(f, "%.15lf\n", res_.back());
    fclose(f);
  }
...
void cross_platform_reproducibility(cxxopts::ParseResult &result,
  ...
  if (!check_ok) {
    fmt::print("shared mem two steps: FAILED\n");
    fmt::print("difference in arithmetic coming most likely from optimization on the CPU side\n");
  } else {
...
void generate_variability_distribution(
  ...
  if (!result["variability_distribution"].as<bool>())
    return;
  generator_t gen;
  double *scratch_;
      ...
      for (int sample = 0; sample < data_.size(0); sample++) {
        ...
        double result_non_det_ =
            reduce(reduction_method::single_pass_gpu_shared_atomic,
                   ...
                   data_.at<device_t::GPU>(sample, 0),
                   static_cast<double *>(scratch_));
        result_stat_(sp * data_.size(0) + sample, step) = variability(result_non_det_, det_sum_(sample, step));
...
int main(int argc, char **argv) {
  ...
  options.add_options()("S,number_of_samples",
      ...
      "performance_tests",
      "Measure timings for reductions of fixed sizes",
      cxxopts::value<bool>()->default_value("false"))(
      "variability_distribution",
      "Calculate the variability distribution",
      cxxopts::value<bool>()->default_value("false"))(
      "variability_under_load",
      "Calculate the variablity distribution under load",
      cxxopts::value<bool>()->default_value("false"))(
     ...
     "mapping_under_load",
     "measure the mapping between the atomicInc execution order and the block "
     "index when other wrok runs in parallel. ",
     cxxopts::value<bool>()->default_value("false"))(
     "append",
     "append the results at the end of the csv file and create it if it does not exist",
     cxxopts::value<bool>()->default_value("false"))(
                                                     "matrix_size",
                                                     "size of square matrix. Only relevant when mapping or mapping_under_load is called",
  ...
  */
  ...
  timer_.start("measure_varibility_under_load");
  measure_variability_under_load(result, timer_, stream_vector_);
  timer_.stop("measure_varibility_under_load");

  timer_.start("calculate_mapping");

  printf("%d\n", result["mapping"].as<bool>());
  if (result["mapping"].as<bool>())
    measure_atomic_ordering_under_load(result, false, timer_, stream_vector_);
  timer_.stop("calculate_mapping");
  
  timer_.start("calculate_mapping_under_load");
  if (result["mapping_under_load"].as<bool>())
    measure_atomic_ordering_under_load(result, true, timer_, stream_vector_);
  timer_.stop("calculate_mapping_under_load");
