...
__inline__ void blasCreate(blas_handle_t *ctx__)
{
  assert(ctx__ != nullptr);
#ifdef REDUCE_USE_CUDA
  cublasCreate(ctx__);
#else
  hipblasCreate(ctx__);
...
__inline__ void blasDestroy(blas_handle_t ctx__)
{
#ifdef REDUCE_USE_CUDA
  cublasDestroy(ctx__);
#else
  hipblasDestroy(ctx__);
#endif
}

__inline__ void blasSetStream(blas_handle_t handle__, whip::stream_t streamId) {
#ifdef REDUCE_USE_CUDA
  cublasSetStream(handle__, streamId);
#else
  hipblasSetStream(handle__, streamId);
#endif
}

__inline__ void randSetStream(generator_t gen__, whip::stream_t streamId) {
#ifdef REDUCE_USE_CUDA
  curandSetStream(gen__, streamId);
#else
  hiprandSetStream(gen__, streamId);
#endif
}
