...
template <class T, unsigned int blockSize> __global__ void
multi_dimensional_reduce_gpu(const T *__restrict__ g_idata,
                             unsigned int *refCount__,
                             const unsigned int *size_array__,
                             const unsigned int *offset__,
                             int *block_index__,
                             T *g_odata) {
  // Handle to thread block group
  T *sdata = SharedMemory<T>();
  // perform first level of reduction, reading from global memory, writing to
  // shared memory
  ...
  unsigned int *retirementCount = refCount__ + blockIdx.y;
  const int offset = offset__[blockIdx.y];
  const int size_array = size_array__[blockIdx.y];
  T mySum = 0;
  // we reduce multiple elements per thread.  The number is determined by the
  // number of active thread blocks (via gridDim).  More blocks will result
  // in a larger gridSize and therefore fewer elements per thread
  while (i < size_array) {
    mySum += g_idata[i + offset];

    // ensure we don't read out of bounds -- this is optimized away for powerOf2
    // sized arrays
    if (i + blockSize < size_array)
      mySum += g_idata[i + blockSize + offset];
  ...
  if (gridDim.x > 1) {
    ...
    // wait until all outstanding memory instructions in this thread block are
    // Finished
    __threadfence();
    ...
    if (tid == 0) {
      unsigned int ticket = atomicInc(retirementCount, gridDim.x);
      if (block_index__)
        block_index__[blockIdx.y * gridDim.x + ticket] = blockIdx.x;
      // If the ticket ID is equal to the number of blocks, we are the last
      // block!
      amLast = (ticket == gridDim.x - 1);
    }
    ...
    // The last block sums the results of all the other blocks
    if (amLast) {
      i = tid;
      mySum = (T)0;

      while (i < gridDim.x) {
        mySum += g_odata[i + blockIdx.y * gridDim.x];
        i += blockDim.x;
      }
...
// template <class T> void reduce_multidimensional_array(whip::stream_t stream__,
//                                                       const int block_size,
//                                                       const int grid_size,
//                                                       const int num_arrays__,
//                                                       const int *array_size__,
//                                                       const int *offset__,
//                                                       const T *d_idata,
//                                                       int *refCount__,
//                                                       T *d_odata,
...
//   dim3 dimGrid(grid_size__, num_arrays__, 1);
//   const int shared_mem = block_size * sizeof(T);
  
//   switch(block_size) {
//   case 512:
//     multi_dimensional_reduce_gpu<T, 512><<<dimGrid, dimBlock, shared_mem, stream__>>>(g_idata,
//                                                                                       refCount__,
//                                                                                       array_size__,
//                                                                                       offset__,
//                                                                                       block_index__,
...
//     break;
//   case 256:
//     multi_dimensional_reduce_gpu<T, 256><<<dimGrid, dimBlock, shared_mem, stream__>>>(g_idata,
//                                                                                       refCount__,
//                                                                                       array_size__,
//                                                                                       offset__,
//                                                                                       block_index__,
...
//     break;
//   case 128:
//     multi_dimensional_reduce_gpu<T, 128><<<dimGrid, dimBlock, shared_mem, stream__>>>(g_idata,
//                                                                                       refCount__,
//                                                                                       array_size__,
//                                                                                       offset__,
//                                                                                       block_index__,
...
template <typename T>
T cub_reduce(whip::stream_t stream__,
...
template <class T>
T reduce_register_atomic(whip::stream_t stream__,
                            const int size__,
                            T *data__,
                            T *scratch__) {
  T *in, *out;
  ...
  // std::vector<T> tmp(grid_size + 1, 0.0);
  whip::memset_async(out, 0, sizeof(T) * (grid_size + 1), stream__);
  reduce_gpu_follow_atomic<T, 64>
      <<<grid_size, block_size, smemSize, stream__>>>(in, out, size__);
  whip::memcpy_async(&result, out, sizeof(T), whip::memcpy_device_to_host,
                      stream__);
  whip::stream_synchronize(stream__);
  return result;
...
// template void reduce_multidimensional_array(whip::stream_t stream__,
//                                             const int block_size,
//                                             const int grid_size,
//                                             const int num_arrays__,
//                                             const int *array_size__,
