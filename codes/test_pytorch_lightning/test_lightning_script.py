...
def main(args):
    ...
    # Setup Logger
    # log_dir: Path = Path(args.log_dir) if args.log_dir else Path(f"{torch.cuda.get_device_name()}/CSV_Logs")    
    ...
    log_path: Path = log_dir / f"{args.experiment_name}" / f"version_{trainer.logger.version}"
