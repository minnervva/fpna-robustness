...
def get_astro_data(train_batch=200, test_batch=1):
    ...
    X_train, X_test, y_train, y_test = train_test_split(data[xcols], data[ycol], random_state=123)
    ...
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size = train_batch)#, num_workers=1)#, shuffle=True)
    test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size = test_batch)#, num_workers=1)
